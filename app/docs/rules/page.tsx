    ...
    return (
        <section className="container w-fit space-y-4">
            ...
            {ruleCategories.map((category, categoryIndex) => (
                ...
                >
                  {getMarkerContent(categoryIndex, ruleIndex)}
                                ...
                                {ruleObj.rule}
                                {ruleObj.description && (
                                    <p className="text-sm text-gray-500">{ruleObj.description}</p>
                                )}
                            </li>
                        ))}
                    </ol>
            ...
            <div className="alert alert-warning shadow">
                <Book />
                <span>
          Remember: WorldMC staff has the final say in rule interpretation and
          enforcement. Play responsibly and have fun!
        </span>
            </div>
        </section>
    );
